  ...
  if (func_name == "lower" || func_name == "upper") {
    if (args.size() == 1) {
      if (args[0]->GetReturnType() == TypeId::VARCHAR) {
        return static_cast<AbstractExpressionRef>(std::make_shared<StringExpression>(
            args[0], (func_name == "lower" ? StringExpressionType::Lower : StringExpressionType::Upper)));
      }
      throw Exception(fmt::format("the type of func call {} should be VARCAHR", func_name));
    }
    throw Exception(fmt::format("the number of func call {} should be 1", func_name));
  }
